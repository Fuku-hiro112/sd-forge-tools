    ...
    def ui(self, *args, **kwargs):
        ...
        with gr.Accordion("$ Prompt Expander", open=False):
            ...
            with gr.Accordion("📖 記法・使い方", open=False):
                ...
                with gr.Accordion("⑤-b {x;y;...} ランダム選択グループ", open=False):
                    gr.Markdown(
                        "`;` と `//` は直積展開（全組み合わせを網羅して複数枚生成）ですが、\n"
                        "`{ }` で囲むと**生成される画像ごとに独立してランダムに1つだけ**選ばれ、"
                        "生成枚数には影響しません。\n\n"
                        "**例**\n"
                        "```\n"
                        "a;b//{c;d}//e;f\n"
                        "```\n"
                        "`a/b` と `e/f` の直積で **4 枚**生成されます（`ae` `af` `be` `bf`）。\n"
                        "中央の `{c;d}` は各画像で独立に `c` か `d` のどちらかがランダムに選ばれ、"
                        "4 枚という枚数自体は変わりません。\n\n"
                        "- `//...//` の中でも本文中でも使えます。\n"
                        "- `$変数` を中に書くこともできます（例: `{$a;$b}`）。\n"
                        "- **`;` を含まない `{...}`（選択肢が1つ）は無変更のまま出力されます。**\n"
                        "  旧 `{1}{2}` 記法や `{強調語}` のような単体の中括弧と衝突しないための仕様です。\n"
                    )

                with gr.Accordion("⑥ 廃止記法", open=False):
                    gr.Markdown(
                        "- 旧 `{1}{2}` 記法は廃止。検出時は警告ログのみ出力。\n"
                        "- `{1}` のように **`;` を含まない波括弧は、⑤-b のランダム選択グループの対象外**のため、"
                        "従来通り中身は変更されずそのまま出力されます。\n"
                    )
